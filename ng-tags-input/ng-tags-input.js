        ...
        controller: ["$scope","$attrs","$element", function($scope, $attrs, $element) {
            ...
            this.registerAutocomplete = function() {
                ...
                input.on('keydown', function(e) {
                    ...
                    //MF, move the position of auto complete
                    var top = $(input).position().top;
                    var left = $(input).position().left;
                    var topAuto = top+20;
                    $('tags-input .autocomplete').css({'left':left+5+'px', 'top':topAuto+'px'});
                    if (left>200) {
                        $('tags-input .autocomplete').css({'left':left-150+'px', 'top':topAuto+'px'});
                    }
                });
        ...
        link: function(scope, element, attrs, ngModelCtrl) {
            ...
            scope.$watch('tags.length', function() {
                ...
                if (scope.tags.length==0) {
                    if (scope.options.maxTags==1) {
                        $(input).show();
                        input.attr("placeholder", "Type a Contact");
                    }
                    else
                        input.attr("placeholder", "Type Contact or Group Name");
                    $(input).width(200);
                } else {
...
tagsInput.run(["$templateCache", function($templateCache) {
    ...
    "<div class=\"host\" tabindex=\"-1\" ti-transclude-append=\"\"><div class=\"tags\" ><ul class=\"tag-list\"><li class=\"tag-item\" ng-repeat=\"tag in tagList.items track by track(tag)\" ng-mouseover=\"tag.showRemove=true\" ng-mouseleave=\"tag.showRemove=false\" ng-class=\"{ selected: tag == tagList.selected }\"><span ng-bind=\"getDisplayText(tag)\"></span> <a class=\"remove-button\" ng-click=\"tagList.remove($index)\" data-ng-show=\"tag.showRemove&&!tag.canRemove\"><img src='ng-tags-input\\red-cross.png'/></a></li></ul><input class=\"input\" ng-model=\"newTag.text\" ng-change=\"newTagChange()\" ng-trim=\"false\" ng-class=\"{'invalid-tag': newTag.invalid}\" ti-bind-attrs=\"{type: options.type, placeholder: '', tabindex: options.tabindex}\" ti-autosize=\"\"></div></div>"
  );
