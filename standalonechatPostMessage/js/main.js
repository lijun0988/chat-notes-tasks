var jabberConfig = {
	...
	httpBindingURL: location.protocol+'//' + location.hostname + (location.port ? ':' + location.port : '') + "/httpbinding/", //the BOSH url for your server
...
var onPresenceReceived = function (event) {
    ...
    if (bareJidStr && bareJidStr != client.connectedUser.jid.getBareJIDString()) {
		...
		if (  $.inArray(key, openWindows) != -1 ) {
			chatWindows[key].postMessage({'type':'theirpresence', 'status':users.list[key].status}, 'http://localhost');
		}		
		
		rosterList.users[key] = entity;
		console.log(key + ' entity: ' + entity);
    } else {
		$.each(openChatWindows(), function (i, key ) {
			chatWindows[key].postMessage({'type':'mypresence', 'status': currentUser.status}, 'http://localhost');
		});
	}
}
...
var createChatWindow = function (user, message) {
	var openWindows = openChatWindows();
	if (  $.inArray(user.username, openWindows) == -1 ) {
		var strWindowFeatures = "menubar=no,location=no,resizable=yes,scrollbars=yes,status=yes,height=480,width=400";
		var queryString = '?user=' + user.username +'&fullname=' + userNames.getName(user.username);
		if ( message ) {
			queryString += "&message=" + message;
		}
		
		chatWindows[user.username] = window.open('chat.html'+queryString, user.username, strWindowFeatures);
		var startMessage = function () {
			chatWindows[user.username].postMessage(
				{
					'type':'enablechat', 
					'currentUser': {
					...
					'theirStatus': users.list[user.username].status,
					'user': user.username
				}, 'http://localhost');
			chatWindows[user.username].focus();
			console.log('starting chat with ' + user.username + '...');
		}
		window.setTimeout(startMessage, 500);
...
var messageReceived = function (evt) {
	...
	if (body) {
		...
		var openWindows = openChatWindows();
		if (  $.inArray(sender, openWindows) == -1 ) {
			createChatWindow(users.list[sender], body);
		} else {
			chatWindows[sender].postMessage( {'message': body ,'type':'chat'}, 'http://localhost');
		}
	}
}
...
var sendMessage = function (chatWith, message, time) {
	var valid = true;
	var jid;
	try {
		jid = jabberwerx.JID.asJID(chatWith);
		if (!jid.getNode() && !jid.getResource()) {
			jid = jabberwerx.JID.asJID(jid.toString() + '@' + jabberConfig.domain + '/' + client.resourceName);
		}
	} catch (ex) {
		valid = false;
		jabberwerx.util.debug.warn("This is not a valid JID or node");
		jabberwerx.util.debug.warn(chatWith);
	}
	if ( valid ) {
		client.sendMessage(jid.toString(), message);
		console.log('message to ' + jid + ': ' + message);
		var from = client.connectedUser.jid.getBareJID().getNode();
		logMessage( from, chatWith, message, time );
	}	
}
...
jabberwerx.$(document).ready(function() {
	...
	var getSendMessage = function (event) {
		var data = event.data;
		if ( typeof data.type != 'undefined' && data.type == 'sendchat' && data.chat.from == currentUser.username) {
